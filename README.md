...
Project Lab's pixel lettering uses local CC BY-SA 3.0 font recreations by lou for English and OFL 1.1 Fusion Pixel fonts for Japanese and Hong Kong Traditional Chinese; retain their [font notices](assets/webfonts/project-journal/README.md). The CJK fonts load when their language is selected. Its [pixel control artwork](images/project-journal/controls/README.md) is local SVG drawn on fixed pixel grids. The journal's chrome overrides live in `desktop/styles.css` and apply only to its window; the page title, body copy, and other sites keep their existing fonts. There is no monitor overlay or added runtime dependency.
