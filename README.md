...
Project Lab's pixel lettering uses local CC BY-SA 3.0 font recreations by lou for English and OFL 1.1 Fusion Pixel fonts for Japanese and Hong Kong Traditional Chinese; retain their [font notices](assets/webfonts/project-journal/README.md). The CJK fonts load when their language is selected. The retro lettering, controls, and immediate hover/pressed states are scoped to `.project-journal-content` below the shared browser header; the title bar, menus, toolbar, and address row retain the common browser styling. The page title and body copy keep their existing fonts. Its status selector uses local [pixel control artwork](images/project-journal/controls/README.md). There is no monitor overlay or added runtime dependency.
