...
Start with the [README file map](../README.md#where-to-edit). Portfolio, Personal Hobbies, Project Lab, and My Websites each keep their content, styles, and behavior under `sites/`; DVD Project Lab is archived in `sites/archieve/projects/`. Hobbies, Project Lab, and My Websites share an Internet Explorer window header maintained in `desktop/`; the portfolio keeps its own Explorer frame.

## How a page reaches the browser

`desktop/page.html` contains the outer page, the portfolio frame, and markers such as `<!-- browser-window: hobbies -->`. `scripts/browser_content.py` expands each browser-window marker using `desktop/browser-window.html` and its site's `BrowserWindow(id, title, home_id)` configuration in `scripts/site_config.py`. The build then inserts each enabled site's `content.html` through the usual include markers. The directory's `<!-- website-shortcuts -->` marker is rendered from `sites/my-websites/websites.json` and its shared `shortcut.html` template by `scripts/website_content.py`. `scripts/project_journal_content.py` renders the active Project Lab journal from `sites/project-journal/projects.json`, its three locale files, localized `excerpts.json`, the site-owned `templates/` markup, and the local photo manifest. If DVD Project Lab is restored and enabled, `scripts/project_content.py` renders its `<!-- project-catalogue -->` marker from `sites/projects/projects.json`. The renderers run through `scripts/build_site.py`; no JSON is fetched by the browser. Running `python3 scripts/build_site.py` writes the complete root `index.html`, `assets/js/site-registry.js`, `assets/css/one-page.css`, and `sites/project-journal/translations.js`.
...
The DVD Project Lab entry in `scripts/site_config.py` is disabled. The paired `<!-- project-lab:start -->` / `<!-- project-lab:end -->` blocks omit its window, Start entry, and taskbar button before includes are expanded. The directory record's `"feature": "project-lab"` uses the same site configuration to omit its shortcut. The build also omits its JavaScript and CSS imports, so moving its folder into the archive does not break startup. Disabled DVD URLs fall back to the portfolio, and the no-JavaScript view cannot expose the omitted content. To restore it, move `sites/archieve/projects/` back to `sites/projects/`, enable the DVD entry in `scripts/site_config.py`, restore `sites/projects` in the security checker's module allowlist, and rebuild. Other sites remain enabled until they have equivalent HTML gates; the build rejects disabling them prematurely. Follow the archive's `note.md` while it remains archived. The active journal has an independent `project-journal` window and `#project-journal...` routes outside every DVD feature gate.
...
- **Project Lab journal:** `sites/project-journal/` owns its searchable project list, development notes, roadmaps, photo credits, language controls, and `#project-journal...` destinations. Canonical facts and relationships live in its `projects.json`; locale JSON contains text overlays. Its local module translates existing DOM text, preserves the selected project and filters, and delegates all window controls to the desktop.
- **Archived DVD Project Lab:** the preserved DVD animation, expandable project workspace, catalogue filtering, selection, roadmaps, and updates; disabled by default.
...
- **Update Project Lab:** edit `sites/project-journal/projects.json` once for facts, relationships, statuses, dates, and links, then update the matching text overlays in `locales/en.json`, `locales/ja.json`, and `locales/zh-HK.json`. Use stable lowercase hyphenated project, milestone, and update IDs so direct links survive edits. Match every project, milestone, update, and link label in each locale; the English overlay must match canonical English. Keep unknown update dates and blockers `null`, use actual supplied/verified dates, and omit unconfirmed facts. The build validates and escapes the sources, derives the project list/count/details, and generates the English fallback and `translations.js`; never hand-edit that module or fetch the JSON in the browser. Keep `excerpts.json` aligned with the project and update IDs in all three languages; it supplies concise previews while native disclosures retain the complete original text. Null update dates are omitted. The journal's snapshot is independent from the archived DVD data.
- **Preserve Project Lab photographs:** keep the three display JPEGs, `photos.json`, `PHOTO_CREDITS.md`, and `PHOTO_PROCESSING.md` under `images/project-journal/`. Keep source, creator, license, capture-date, and modification records accurate in both the manifest and visible credits. S01 was captured in 2016; T01 in 2008 and is explicitly color/tone edited under CC BY-SA 4.0; H01's exact capture date is unknown. If a file or crop changes, update its changes notice and recorded metadata. Source/license links are visitor-initiated; photographs stay local.
- **Restore or update DVD Project Lab:** move `sites/archieve/projects/` back to `sites/projects/`, enable it in `scripts/site_config.py`, restore `sites/projects` in `.github/scripts/check_site_security.py`'s module allowlist, and rebuild. Once restored, edit `sites/projects/projects.json` for catalogue changes. Preserve stable project, milestone, and update IDs. Use the supported project/milestone statuses; optional update dates use `YYYY-MM-DD` or `null`. Omit unconfirmed links and facts. The renderer validates relationships, IDs, dates, and HTTPS links and escapes content. Search labels, project rows, roadmaps, related links, and update notes come from that same catalogue.
...
7. Run `python3 scripts/build_site.py`, commit the generated `index.html`, `assets/js/site-registry.js`, `assets/css/one-page.css`, and `sites/project-journal/translations.js` with their sources, and follow the checks below. Also check a direct link to the new site, links between sites, directory discovery, and the no-JavaScript fallback. Check that lists still work with additional entries and long names. Update the README file map when adding the folder.
...
- `index.html`, `assets/js/site-registry.js`, `assets/css/one-page.css`, and `sites/project-journal/translations.js`: generated outputs; rebuild them instead of editing them.
