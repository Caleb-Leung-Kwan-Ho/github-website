...
def render_translations(root: Path) -> str:
    ...
    return ("// Generated by scripts/build_site.py; edit projects.json and locales/*.json.\n"
    ...
    for project in projects:
        ...
        if project["updates"]:
            ...
            for update in project["updates"]:
                stamp = f'<time datetime="{update["date"]}">{update["date"]}</time>' if update["date"] else label("dateNotRecorded")
                update_list += [f'                <li id="{base}-update-{update["id"]}">', f'                  <p class="journal-update-date">{stamp}</p>', f'                  <p>{field(project, "text", kind="update", item=update)}</p>', '                </li>']
        ...
        if project["milestones"]:
            ...
            for milestone in project["milestones"]:
                description = f'<p>{field(project, "description", kind="milestone", item=milestone)}</p>' if "description" in milestone else ""
                rows.append(f'                  <tr id="{base}-milestone-{milestone["id"]}"><th scope="row">{field(project, "title", kind="milestone", item=milestone)}{description}</th><td>{status(milestone["status"])}</td></tr>')
